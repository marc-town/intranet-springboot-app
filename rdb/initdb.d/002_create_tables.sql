...
(
    ...
    `entered_date` date not null default current_date comment '入社日',
...
(
    ...
    `staff_id` int not null `社員ID`,
    `staff_type_id` int not null default 0 comment '社員種別ID',
    `birthday` varchar(10) default 'yyyy-mm-dd' comment '誕生日',
    `telephone_number` varchar(13) default 'xxx-yyyy-zzzz' comment '電話番号',
    `mail_address` varchar(255) 'mailadress@example.com' comment 'メールアドレス',
    ...
    `update_at` timestamp not null default current_timestamp on update current_timestamp comment '更新日',
    PRIMARY KEY(`id`),
    CONSTRAINT fk_staff_id  -- 制約の名前
        FOREIGN KEY (staff_id)  -- 外部キーに設定する絡む名
        REFERENCES m_staff (staff_id)  -- 参照するテーブルと絡む
        DELETE SET NULL ON UPDATE CASCADE  -- 参照先（親テーブル）をdeleteするとNull、updateすると子もupdate
) DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
